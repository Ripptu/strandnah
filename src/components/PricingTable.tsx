...
export default function PricingTable({ seasonalPrices }: PricingTableProps) {
    ...
    <div className="py-8 border-b border-border-light text-text-primary">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        {sortedSeasons.map((season, sIdx) => {
          const customBase = seasonalPrices?.[season.name]?.basePrice ?? season.basePrice;
          const customWeekend = seasonalPrices?.[season.name]?.weekendPrice ?? season.weekendPrice;
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-2xl p-6 flex flex-col sm:flex-row items-start gap-4">
        ...
        <div className="text-sm text-text-secondary space-y-2">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>Die jeweiligen Wochenendpreise gelten für Buchungen, die Freitags und Samstags Nächte beinhalten.</li>
